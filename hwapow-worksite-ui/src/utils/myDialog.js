...
common.dialog = function(option) {
  ...
  if (option.parentel !== undefined && option.parentel !== null) {
    option.parentel.appendChild(m)
  } else {
    document.getElementsByTagName('body')[0].appendChild(m)
  }
  let _d = common.clone(option.data == null ? {} : option.data)
  let optionData = {
    title: option.title,
  ...
  if (typeof component === 'string') {
    ...
    template = '<div class="w680" v-if="show">' +
      '<el-dialog :style="style" :title="title" v-model="show" :size="size" :before-close="handleClose" @close="close">' +
      '<child v-on:callback="callback" :init_data="data"></child>' +
      '</el-dialog></div>'
  }

  let v = '';
  if (option.type && option.type === 'iframe') {
    optionData.url = option.url
